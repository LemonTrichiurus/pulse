...
export default function EditNewsPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setForm({
          ...
          content_rich: newsData.content,
          cover_url: newsData.featured_image || '',
          category: newsData.category,
          status: newsData.status,
          top_rank: newsData.top_rank ?? null
        })
      } else {
        toast.error('新闻不存在或无权访问')
  ...
  const handleInputChange = (field: keyof NewsForm, value: any) => {
    setForm(prev => ({
      ...prev,
      [field]: value
    }))
  ...
  const handleSubmit = async (newStatus?: 'DRAFT' | 'PUBLISHED') => {
    ...
    try {
      ...
      if (response.ok) {
        const action = newStatus === 'PUBLISHED' ? '发布' : newStatus === 'DRAFT' ? '保存为草稿' : '更新'
        toast.success(`新闻${action}成功`)
        router.push('/admin/news')
      } else {
        const error = await response.json()
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h2 className="text-xl font-semibold mb-2">新闻不存在</h2>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>修改新闻的基本信息</CardDescription>
            </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    if (v === '') return handleInputChange('top_rank', null)
                    const n = Number.parseInt(v, 10)
                    if (Number.isNaN(n)) return
                    handleInputChange('top_rank', n)
                  }}
                />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>上传或更换新闻封面图片</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {form.cover_url ? (
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>编辑新闻的详细内容</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
